...
const SimpleImageEditor = React.forwardRef(function SimpleImageEditor(
  ...
  const replaceWithCanvasImage = React.useCallback(
    async url => {
      ...
      stateRef.current.objects = [
        makeCanvasObject(
          `object-${stateRef.current.nextId++}`,
          url,
          img,
          size.width,
          size.height,
    ...
    const editor = {
      ...
      execute: (command, id) => {
        if (command === 'removeObject') {
          ...
          if (stateRef.current.activeId === id) stateRef.current.activeId = null
          render()
          emit('undoStackChanged')
        }
      },
  ...
  const handleMouseMove = event => {
    ...
    ctx.strokeStyle = brush.color.startsWith('#') ? brush.color : `#${brush.color}`
